...
func main() {
	...
	ins, err := dubbo.NewInstance(
		dubbo.WithName("dubbo_registry_nacos_group_client"),
		dubbo.WithRegistry(
			registry.WithNacos(),
	...
	resp, err := svc.Greet(context.Background(), &greet.GreetRequest{Name: "group hello world"})
	if err != nil {
		logger.Error(err)
	}
	logger.Infof("Group Greet response: %s", resp)
	select {}
}