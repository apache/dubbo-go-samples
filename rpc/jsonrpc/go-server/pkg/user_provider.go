...
func (u *UserProvider) GetUser2(ctx context.Context, req *User) (*User, error) {
	var err error
...
func (u *UserProvider) GetUsers(req []User) ([]User, error) {
	var err error

	gxlog.CInfo("req:%s", req)
	user, err := u.getUser(req[0].ID)
	if err != nil {
		return nil, err
	}
	gxlog.CInfo("user:%v", user)
	user1, err := u.getUser(req[1].ID)
	if err != nil {
		return nil, err
	}
