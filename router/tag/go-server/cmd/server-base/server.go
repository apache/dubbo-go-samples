...
func main() {
	...
	if err := greet.RegisterGreetServiceHandler(srv, &GreetServer{srvName: "server-without-tag"}); err != nil {
	...
	if err := srv.Serve(); err != nil {
		...
		if strings.Contains(err.Error(), "client not connected") {
			logger.Errorf("hint: Nacos client not connected (gRPC). Check %s is reachable and gRPC port %d is open (Nacos 2.x default).", RegistryAddress, 20000)
		}
		panic(err)
	}
