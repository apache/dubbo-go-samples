...
- **go-server-game**: 处理游戏逻辑的游戏服务（Login、Score、Rank）
- **go-server-gate**: 为 Web 前端提供 HTTP API 并为游戏服务提供 RPC 服务的网关服务
- **website**: 足球游戏的 Web 前端
- **proto**: 游戏和网关服务的 Protocol Buffer 定义

## 目录结构

- `go-server-game/cmd/main.go` - 游戏服务服务器实现
- `go-server-game/pkg/provider.go` - 游戏服务处理器实现
- `go-server-gate/cmd/main.go` - 带 HTTP 和 RPC 的网关服务服务器
- `go-server-gate/pkg/provider.go` - 网关服务处理器实现
- `go-server-gate/pkg/consumer.go` - 网关服务调用游戏服务的客户端
