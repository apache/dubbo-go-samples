...
- Unary response headers and trailers with `triple.SetHeader`, `triple.SendHeader`, `triple.SetTrailer`, `client.WithResponseHeader`, and `client.WithResponseTrailer`
...
For generated Dubbo-Go unary calls, the server uses `triple.SetHeader`, `triple.SendHeader`, and `triple.SetTrailer` to attach response metadata, and the client passes `client.WithResponseHeader` and `client.WithResponseTrailer` to capture it for that call. `SetHeader` buffers the headers until the response is sent; `SendHeader` flushes them immediately (before the first message), which is useful when a handler needs to publish response metadata early. For generated bidi-stream and client-stream calls, pass request metadata before stream creation with `NewOutgoingContext` / `AppendToOutgoingContext`. On the server side, handlers can read the resulting metadata from `triple.FromIncomingContext`; generated stream handlers can also inspect it through `RequestHeader`.
